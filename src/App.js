      ...
      <div className="App">
        ...
        {txs && (
          <div className="results">
            {txs.result?.map((e, i) => {
              return (
              <a href={`https://mumbai.polygonscan.com/tx/${e.hash}`} key={i} className="tx" target="_blank" rel="noreferrer">
                [{e.block_timestamp.slice(0,10)}]
                {e.from_address.toLowerCase() === address.toLowerCase() ?
                <span> Send {(e.value / 1E18).toFixed(4)} ETH {e.value.from_address }</span>  :
                <span> Receive {(e.value / 1E18).toFixed(4)} ETH</span>
              }
              </a>
              )
            })}
